...
3. Vision-модели через выбранный профиль Connection Manager уходят два сообщения: `system` (промпт из настроек, `{NAMES}` заменяется
   полем «Имена», в конец дописывается блок `Available LoRA sets:` из каталога бриджа — по строке `name — triggers — hint` на сет)
...
4. Попап (`callGenericPopup`), кнопки **Го / Отмена**:
   ...
   * **Секунды**, **Разрешение** 480/720, **Seed** (пусто = случайный).
...
| Системный промпт | Текст system-сообщения для vision-модели (без блока каталога — он дописывается сам), кнопка **сброс** возвращает промпт по умолчанию. | см. `DEFAULT_SYSTEM_PROMPT` в `index.js` |
